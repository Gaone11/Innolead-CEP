...
export default function DiagnosticView({ setActiveView }: DiagnosticViewProps) {
  ...
  const overallScore = Math.round(
    sections.reduce((sum, s) => sum + getSectionScore(s.id), 0) / sections.length
  );
      ...
      <div className="fade-in-up">
        <div style={{
          ...
          <div className="glow-cyan" style={{
            ...
            <Brain size={36} color="#0F1419" />
          </div>
          <h2 style={{ fontFamily: "Montserrat, sans-serif", marginBottom: 8 }}>Assessment Complete</h2>
          <p style={{ color: "#9CA3AF", marginBottom: 24 }}>Your Diagnostic & Scoring Agent has processed your responses</p>
          ...
          <div style={{ position: "relative", width: 160, height: 160, margin: "0 auto 28px" }}>
            <svg viewBox="0 0 160 160" style={{ transform: "rotate(-90deg)", width: "100%", height: "100%" }}>
              <circle cx="80" cy="80" r="65" fill="none" stroke="#252B3A" strokeWidth="14" />
              <circle
                cx="80" cy="80" r="65" fill="none"
                stroke="#3BC2FB" strokeWidth="14"
                strokeDasharray={`${2 * Math.PI * 65}`}
                strokeDashoffset={`${2 * Math.PI * 65 * (1 - overallScore / 100)}`}
                strokeLinecap="round"
                style={{ transition: "stroke-dashoffset 1.5s ease-in-out" }}
              />
            </svg>
            <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
              <div style={{ fontSize: 36, fontFamily: "Montserrat, sans-serif", fontWeight: 800, color: "#fff" }}>{overallScore}%</div>
              <div style={{ fontSize: 12, color: "#9CA3AF" }}>Maturity</div>
            </div>
          </div>
          ...
          <p style={{ color: "#9CA3AF", fontSize: 14, maxWidth: 480, margin: "0 auto 28px" }}>
            {overallScore >= 70
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 14, maxWidth: 520, margin: "0 auto 28px", textAlign: "left" }}>
            {sections.map(s => {
              ...
              return (
                <div key={s.id} style={{ backgroundColor: "#252B3A", borderRadius: 12, padding: 16 }}>
                  <div style={{ fontSize: 12, color: "#9CA3AF", marginBottom: 8 }}>{s.label}</div>
                  <div style={{ height: 4, backgroundColor: "#374151", borderRadius: 2, marginBottom: 6 }}>
          ...
          <div style={{ display: "flex", gap: 12, justifyContent: "center" }}>
            <button
              ...
              style={{ padding: "12px 28px", borderRadius: 10, border: "none", cursor: "pointer", fontSize: 14, display: "flex", alignItems: "center", gap: 8 }}
            >
  ...
  return (
    <div className="fade-in-up">
      {/* Progress header */}
      <div style={{ backgroundColor: "#1A1F2E", border: "1px solid #374151", borderRadius: 14, padding: "18px 22px", marginBottom: 20 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
          <div>
            <h3 style={{ margin: 0, fontFamily: "Montserrat, sans-serif", fontSize: 14 }}>Organisational Maturity Diagnostic</h3>
            <p style={{ margin: 0, fontSize: 12, color: "#9CA3AF" }}>{answeredCount} of {totalQuestions} questions answered</p>
          </div>
          <div style={{ fontSize: 13, color: "#3BC2FB", fontFamily: "Montserrat, sans-serif", fontWeight: 700 }}>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  background: i === currentSection ? `${s.color}18` : "#252B3A",
                  cursor: "pointer",
                  borderBottom: i === currentSection ? `2px solid ${s.color}` : "2px solid transparent",
                  display: "flex", alignItems: "center", justifyContent: "center", gap: 6,
                  color: i === currentSection ? s.color : "#6B7280", fontSize: 12,
                  fontFamily: "Montserrat, sans-serif", fontWeight: 600,
                  transition: "all 0.2s",
                }}
              >
      ...
      {/* Questions */}
      <div style={{ backgroundColor: "#1A1F2E", border: "1px solid #374151", borderRadius: 14, padding: 28, marginBottom: 20 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 24 }}>
          <div style={{
            width: 10, height: 10, borderRadius: "50%", backgroundColor: section.color,
          }} />
          <h3 style={{ margin: 0, fontFamily: "Montserrat, sans-serif", fontSize: 16, color: section.color }}>
            {section.label}
          </h3>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 28 }}>
          {section.questions.map((q, qi) => (
            <div key={q.id} className="slide-in-left" style={{ animationDelay: `${qi * 0.1}s` }}>
              <p style={{ margin: "0 0 14px", fontSize: 14, color: "#E5E7EB", lineHeight: 1.6, fontWeight: 500 }}>
                <span style={{ color: section.color, fontFamily: "Montserrat, sans-serif", fontWeight: 700, marginRight: 6 }}>
                  Q{qi + 1}.
                </span>
                {q.text}
              </p>
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        border: selected ? `2px solid ${section.color}` : "1px solid #374151",
                        background: selected ? `${section.color}18` : "#252B3A",
                        color: selected ? section.color : "#9CA3AF",
                        cursor: "pointer", fontSize: 11,
                        fontFamily: "Montserrat, sans-serif", fontWeight: selected ? 700 : 400,
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <button
          ...
          style={{
            ...
            color: currentSection === 0 ? "#4B5563" : "#E5E7EB",
        ...
        {currentSection < sections.length - 1 ? (
          <button
            onClick={() => sectionAnswered && setCurrentSection(currentSection + 1)}
            className={sectionAnswered ? "btn-primary" : ""}
            style={{
              display: "flex", alignItems: "center", gap: 8,
              padding: "11px 20px", borderRadius: 10, border: "none",
              background: sectionAnswered ? undefined : "#252B3A",
              color: sectionAnswered ? undefined : "#4B5563",
          ...
          <button
            ...
            className={allAnswered ? "btn-primary" : ""}
            style={{
              display: "flex", alignItems: "center", gap: 8,
              padding: "11px 24px", borderRadius: 10, border: "none",
              background: allAnswered ? undefined : "#252B3A",
              color: allAnswered ? undefined : "#4B5563",
